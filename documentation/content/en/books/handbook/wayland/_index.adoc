...
[WARNING]
====
If you encounter fonts not showing up in sway or the wf-panel process crashing with error `Truncating shared memory file failed: Invalid argument` or `Unable to create Cairo image surface: invalid value (typically too big) for the size of the input (surface, pattern, etc.)` the expected font package is missing. Install the `freefont-ttf` package by using the command `pkg install freefont-ttf`.
====
